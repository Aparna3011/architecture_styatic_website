    ...
    <>
      <motion.header
        ...
        className={`fixed top-0 left-0 right-0 z-40 transition-all duration-500 ${
          isScrolled ? 'bg-basalt/95 backdrop-blur-md' : 'bg-transparent'
        }`}
      >
        <nav className="mx-auto max-w-[1600px] px-6 md:px-12 lg:px-16">
          <div className="flex h-20 items-center justify-between">
            ...
            <Link to="/" className="relative z-50">
              <span className="font-display text-2xl font-light tracking-[-0.02em] text-ivory">
                SVIT HUB
              </span>
            </Link>
            ...
            <div className="hidden lg:flex items-center gap-10">
              {mainNav.map((item) => (
                <NavLink key={item.href} href={item.href} isActive={location.pathname === item.href}>
                  {item.label}
                </NavLink>
              ))}
            </div>
        ...
        {isMobileMenuOpen && (
          ...
          >
            ...
            >
              ...
              >
                <div>
                  <p className="font-mono text-xs text-stone uppercase tracking-widest mb-2">Studio</p>
                ...
                <div className="flex gap-6">
                  <a href={siteData.social.instagram} className="font-mono text-xs text-ochre uppercase tracking-widest hover:text-gold transition-colors">
                    Instagram
                  </a>
                  <a href={siteData.social.linkedin} className="font-mono text-xs text-ochre uppercase tracking-widest hover:text-gold transition-colors">
                    LinkedIn
                  </a>
                </div>
...
function NavLink({ href, children, isActive }) {
  return (
    <Link
      to={href}
      className="group relative py-2 font-sans text-sm text-ivory/90 hover:text-ivory transition-colors"
    >
      {children}
      <span 
