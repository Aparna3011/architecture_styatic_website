    ...
    >
      <section className="px-6 lg:px-12 mb-16">
        <div className="max-w-7xl mx-auto">
          <motion.div variants={staggerContainer} initial="initial" whileInView="animate" viewport={{ once: true }}>
            <motion.span variants={fadeInUp} className="text-sm tracking-[0.3em] text-primary/60 uppercase mb-6 block">
              Portfolio
            </motion.span>
            <motion.h1 variants={fadeInUp} className="text-5xl md:text-7xl lg:text-8xl font-light leading-[0.9] mb-8">
              Selected<br /><span className="italic text-primary/70">Works</span>
            </motion.h1>
          </motion.div>
        </div>
      </section>
              ...
              {categories.map((category) => (
                <button
                  ...
                  className={`text-sm tracking-[0.15em] uppercase transition-all duration-300 pb-1 border-b ${
                    activeCategory === category ? 'border-primary text-primary' : 'border-transparent text-primary/50 hover:text-primary'
                  }`}
                >
                  {category}
                </button>
      ...
      <section className="px-6 lg:px-12 pb-32">
        <div className="max-w-7xl mx-auto">
                    ...
                    <motion.div key={project.id} initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: index * 0.1 }}>
                      <Link to={`/projects/${project.slug}`} className="group block">
                        ...
                        <div className="flex justify-between items-start">
                          <div>
                            <h3 className="text-2xl font-light mb-2 group-hover:text-primary/70 transition-colors">{project.title}</h3>
                            <p className="text-sm text-primary/60">{project.location} — {project.year}</p>
                          </div>
                          <span className="text-xs tracking-[0.2em] text-primary/40 uppercase">{project.typology}</span>
                        </div>
                      </Link>
                    </motion.div>
                  ))}
                ...
                <motion.div key="list" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="space-y-0">
                  {filteredProjects.map((project, index) => (
                    <motion.div key={project.id} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: index * 0.05 }}>
                      <Link to={`/projects/${project.slug}`} className="group flex items-center justify-between py-8 border-b border-primary/10 hover:border-primary/30 transition-colors">
                        <div className="flex items-center gap-8">
                          <div className="w-32 h-24 bg-primary/5 overflow-hidden flex-shrink-0">
                          ...
                          <div>
                            <h3 className="text-2xl font-light mb-1 group-hover:text-primary/70 transition-colors">{project.title}</h3>
                            <p className="text-sm text-primary/60">{project.location}</p>
                          </div>
                        </div>
                        <div className="flex items-center gap-12">
                          <span className="text-xs tracking-[0.2em] text-primary/40 uppercase hidden md:block">{project.typology}</span>
                          <span className="text-sm text-primary/60">{project.year}</span>
          ...
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.5 }} className="mt-16 pt-8 border-t border-primary/10">
